...
class FaceShapeClassifier:
    ...
    def calculate_jawline(self, landmarks, method="B"):
        ...
        left_jawline_start = self.mid_point(
            landmarks[start_index - 1], landmarks[start_index]
        )
        right_jawline_start = self.mid_point(
            self.landmarks[last_index], self.landmarks[last_index + 1]
        )
        ...
        total_distance += self.calculate_dist(
            landmarks[last_index], right_jawline_start
        )

        # consider method C
        if method == "C":
            total_distance += self.calculate_dist(
                landmarks[start_index], left_jawline_start
            )
    ...
    def top_head_point(self, rect, bottom, mid):
        ...
        """
        y = rect.top()
        x = (mid[0] * (bottom[1] - y) - bottom[0] * (mid[1] - y)) // (
            bottom[1] - mid[1]
        )
        return (x, y)
    ...
    def method_a(
        ...
        if (
            0.8 <= cheek_ratio <= 1.0
            and 0.7 <= jaw_ratio <= 0.8
            and 0.6 <= forehead_ratio <= 0.8
            and 0.3 <= chin_ratio <= 0.4
            and head_ratio <= 1.25
            and jaw_angle <= 50.0
        ):
            result = "ROUND"
        ...
        # oval face
            ...
            0.5 <= cheek_ratio <= 0.8
            and 0.5 <= jaw_ratio <= 0.7
            and 0.5 <= forehead_ratio <= 0.7
            and 0.2 <= chin_ratio <= 0.4
            and 1.25 <= head_ratio <= 1.6
            and jaw_angle > 50.0
        ):
            result = "OVAL"
        ...
        # oblong face
            ...
            0.5 <= cheek_ratio <= 0.8
            and 0.5 <= jaw_ratio <= 0.8
            and 0.5 <= forehead_ratio <= 0.8
            and 0.3 <= chin_ratio <= 0.4
            and head_ratio >= 1.30
            and jaw_angle > 55
        ):
        ...
        # square face
            ...
            0.7 <= cheek_ratio <= 0.99
            and 0.7 <= jaw_ratio <= 0.8
            and 0.6 <= forehead_ratio <= 0.99
            and 0.3 <= chin_ratio <= 0.5
            and head_ratio <= 1.29
            and jaw_angle < 55
        ):
            result = "SQUARE"
    ...
    def method_b(self, forehead_width, cheek_width, face_length, jawline_length):
        ...
        # oval face
        if cheek_vs_length <= 0.95 and forehead_vs_jaw >= 1.05:
            result.append("OVAL")

        # oblong face
        if cheek_vs_length <= 0.95 and 0.95 <= forehead_vs_jaw <= 1.05:
            result.append("OBLONG")

        # round face
        if (
            0.95 <= cheek_vs_length <= 1.0
            and forehead_vs_cheek <= 0.95
            and jawline_vs_cheek <= 0.95
        ):
            result.append("ROUND")

        # square face
        if 0.95 <= cheek_vs_length <= 1.0 and forehead_vs_jaw < 0.9:
            result.append("SQUARE")

        # pear (triangle) face
        if jawline_vs_cheek >= 1.05 and forehead_vs_cheek <= 0.95:
            result.append("TRIANGLE")

        # heart face
        if forehead_vs_jaw > 1.05 and 0.95 <= forehead_vs_cheek <= 1.05:
            result.append("HEART")
    ...
    def method_c(self, forehead_width, cheek_width, face_length, jawline_length, slope):
        ...
        # diamond face
        if cheek_vs_length < 0.8 and forehead_vs_cheek < 0.9 and forehead_vs_jaw > 1.1:
            result.append("DIAMOND")

        # heart face
        if forehead_vs_cheek > 1.2 and forehead_vs_jaw > 1.2:
            result.append("HEART")

        # oblong face
        if (
            cheek_vs_length < 0.79
            and 1.0 <= forehead_vs_cheek <= 1.1
            and 1.0 <= forehead_vs_jaw <= 1.1
        ):
            result.append("OBLONG")

        # oval face
        if (
            0.7 <= cheek_vs_length < 0.79
            and forehead_vs_cheek < 1.0
            and forehead_vs_jaw > 1.0
            and jawline_hard is False
        ):
            result.append("OVAL")

        # round face
        if (
            0.79 <= cheek_vs_length <= 1.1
            and 0.9 <= forehead_vs_jaw <= 1.1
            and forehead_vs_cheek < 0.95
            and jawline_vs_cheek < 0.95
            and jawline_hard is False
        ):
        ...
        # square face
        if (
            0.85 <= cheek_vs_length <= 1.1
            and 0.9 <= forehead_vs_jaw <= 1.1
            and 0.9 <= forehead_vs_cheek <= 1.1
            and jawline_hard is True
        ):
            result.append("SQUARE")

        # pear (triangle) face
        if jawline_vs_cheek > 1.2 and forehead_vs_cheek < 0.9:
            result.append("TRIANGLE")
    ...
    def method_d(self, forehead_width, cheek_width, face_length, jaw_slope):
        ...
        # oblong face
        if cheek_vs_length <= 0.75:
            result.append("OBLONG")

        # heart face
        if (
            0.75 < cheek_vs_length < 0.9
            and
            # jaw_angle < 30 and
    ...
    def apply_method_a(self, debug=True, save_file=None):
        ...
        if debug:
            ...
            cv2.line(
                self.image, cheek_left, cheek_right, color=(0, 255, 0), thickness=1
            )
            ...
            cv2.line(
                self.image,
                bottom_chin,
                (bottom_chin[0], self.face_area.top()),
                color=(0, 255, 0),
                thickness=1,
            )
    ...
    def apply_method_b(self, debug=True, save_file=None):
        ...
        # move forehead line
        dist = (eye_brow_mid[1] - self.face_area.top()) // 2 + (
            temp_line_mid[1] - eye_brow_mid[1]
        )
        ...
        log(
            f"Forehead width: {forehead_width}, Cheek width: {cheek_width}, Face length: {face_length}, Jawline length: {jawline_length}",
            level=2,
        )

        # determine face shape
        face_shape = self.method_b(
            forehead_width, cheek_width, face_length, jawline_length
        )
        ...
        if debug:
            ...
            cv2.rectangle(
                self.image,
                (self.face_area.left(), self.face_area.top()),
                (self.face_area.right(), self.face_area.bottom()),
                (0, 255, 0),
                1,
            )
    ...
    def apply_method_c(self, debug=True, save_file=None):
        ...
        # determine face shape
        face_shape = self.method_c(
            forehead_width, cheek_width, face_length, jawline_length, jawline_slope
        )
    ...
    def apply_method_d(self, debug=True):
        ...
        jawline_slope = self.calculate_jawline_slope(self.landmarks)
        log(
            f"Forehead width: {forehead_width}, Cheek width: {cheek_width}, Face length: {face_length}",
            level=2,
        )
        ...
        # determine face shape
        face_shape = self.method_d(
            forehead_width, cheek_width, face_length, jawline_slope
        )
        return face_shape
